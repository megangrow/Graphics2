...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (int i = 0; i < num_threads-1; ++i) {
            std::thread t{render, std::ref(world), camera, depth, static_cast<double>(samples)/num_threads, num_threads, std::ref(images.at(i)), false};
...
Color trace_path(const World& world, const Ray& ray, int depth) {
    ...
    Color color = texture->value(u, v); // new
    Ray scattered = texture->scatter(ray, hit.value());
...
Color trace_path_iter(const World& world, const Ray& r, int depth) {
    ...
    while (depth > 0) {
        ...
        const Texture* texture = hit->object->texture;
        color *= texture->color;
